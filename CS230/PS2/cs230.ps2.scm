  ...
  (lambda ((lst <list>) (key <key>))
    (let ((n (key-modulus key)))
      (let loop ((rst lst) (prev 0))
        (cond ((null? rst) rst)
              (else
               (let* ((firststep (rsa-transform (car rst) key))
                      (secondstep (- firststep prev))
                      (cur (modulo secondstep n)))
                 (cons cur
...
;; (define test
;;     (rsa-encrypt "test message for Jason Wang" (key-pair-public test-key-2)))

;; (rsa-unconvert-list test (key-pair-private test-key-2))
;;    (14949 14836 4205 13043 14817 13285 4198 14322 4170 12531 14318 4183 12526 13184)

;; (rsa-decrypt test (key-pair-private test-key-2))
;;    "test message for Jason Wang"


;; PROBLEM 2

(define encrypt-and-sign
  (lambda ((toEncrypt <string>) (rPubKey <key>) (sPrivKey <key>))
    (let* ((encrypted (rsa-encrypt toEncrypt rPubKey))
           (compressed (compress encrypted sPrivKey)))
      (cons encrypted (rsa-transform compressed sPrivKey)))))

(define decrypt-and-verify
  (lambda (sMess (rPrivKey <key>) (sPubKey <key>))
    (let* ((message (car sMess))
           (decrypted (rsa-decrypt message rPrivKey))
           (signature (cdr sMess))
           (d-h (rsa-transform signature sPubKey)))
      (if (equal? d-h (compress message rPrivKey))
          decrypted
          #f))))
...
;; PROBLEM 4
;; Wasn't sure what to put here, but basically I just ran
;; (decrypt-and-verify secret-message (crack-rsa NAME) DiffNAME)
;; and did this with each name until I got a successful message which is:
;; (decrypt-and-verify secret-message (crack-rsa Harsh) Bobby)
;; so the sender is Bobby and the receiver is Harsh
;; the output is: "If cryptography is outlawed, only outlaws will have cryptography."

;; PROBLEM 5    
(define pollard
  (lambda ((n <integer>) (base <integer>) (max <integer))
    (letrec ((iter
              (lambda ((r <integer>) (k <integer>))
                (cond ((or (= 1 (car(cdr (cdr (euclid (- r 1) n))))) (= n (car(cdr (cdr (euclid (- r 1) n))))))
                       (iter (exptmod r k n) (+ k 1)))
                      ((<= (- k 1) max) (car(cdr (cdr (euclid (- r 1) n)))))
                      (else #f)))))
      (iter (exptmod base 1 n) 1))))
